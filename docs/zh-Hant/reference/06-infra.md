...
| 層      | 工具                | 範圍                                                    |
| ------- | ------------------- | ------------------------------------------------------- |
| 環境    | **devenv** (2.1.2+) | Shell 啟動、套件、toolchain 鎖定、程序、腳本、git hooks |
| JS 依賴 | **Bun**             | `node_modules`, `bun.lock`                              |

devenv 是單一事實源：`devenv.nix` 宣告 toolchain，`devenv.lock` 鎖定這些工具
所來自的 `nixpkgs` commit。沒有獨立的 flake —— toolchain 版本完全存在
`devenv.lock`。
