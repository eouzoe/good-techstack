...
`check-versions` 是告訴維護者「某個鎖定依賴落後上游」的。另一個檢查則是告訴**你**：你電腦上的工具版本，是否與專案提供的版本一致——因為如果你用了更新的或不同的版本而壞掉，沒人能為後果負責。專案保證這組鎖定的版本能動，而且由我們維護。

```bash
just check-env          # 或：bun scripts/check-env.mjs
```

腳本讀取專案自己的鎖定值——`flake.lock`（nixpkgs 工具：bun、node、oxlint、oxfmt、wrangler、prettier、just）、`devenv.yaml`（devenv CLI）、以及 `bun.lock`（npm CLI，例如 drizzle-kit）——並與你 `PATH` 上的版本比對。出現不一致**不代表「升級」**，而是代表「對齊到專案版本」：

```bash
devenv shell            # 提供恰好鎖定的工具版本
# 或每次 cd 自動進入：
direnv allow
```

對於住在 `bun.lock` 裡的 npm CLI（例如 `drizzle-kit`），用以下方式同步：

```bash
bun install             # 從 bun.lock 拉取鎖定版本
```

不要用 `npm i -g` / 下載最新釋出來「修」不一致。用專案的版本，我們會負責。
