...
- Nix: verify `flake.nix` works on macOS (nix-darwin vs nixpkgs, `nix develop` behaviour)
- Homebrew: if the user does not have Nix, can the stack use Homebrew? Write a `install.sh` fallback
- Direnv: verify `.envrc` `use flake` works on macOS
...
| NixOS | Nix is native but `nix develop` behaviour differs; `/usr/bin/env` not always available | `flake.nix`, shebangs, path assumptions |
...
- Create a `Dockerfile` and `docker-compose.yml` for developers who do not use Nix
- Install Bun, the stack dependencies
- Mount the project directory for HMR
